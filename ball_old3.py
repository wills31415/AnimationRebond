...
class environnement:
    ...
    def __init__(self, res = (800, 400), origine = (0, 0), pixelsPerUnit = 1, coeffRestitutionBord = 1, coeffRestitutionBalle = 1, accelerationPesanteur = 9.807, viscositeDynamique = 0, collisionMovables = False, collisionBorders = False):
    ...
    def update_env(self):
        ...
        # Calcul des forces
        F = {movable : Mat2d([0, 0]) for movable in self.movables}
        for movable in self.movables:
            F[movable] = -self.viscositeDynamique * movable.coeffFrottements * movable.vitesse - movable.masse * self.accelerationPesanteur * Mat2d([0, 1])

        # Résolution des contraintes liées aux liaisons
        for bond in self.bonds:
            if not isinstance(bond.point2, Ball):
                continue
            rotation = Mat2d.rot2d(bond.point2.position - bond.point1.position)
            if not isinstance(bond.point1, Ball):
                if isinstance(bond, Rod):
                    T = rotation * F[bond.point2]
                    T[1] = 0
                    T *= rotation.transpose()
                    F[bond.point2] -= T
                    # F[bond.point2] *= rotation
                    # F[bond.point2][0] = 0
                    # F[bond.point2] *= rotation.transpose()
                    vitesse = rotation * bond.point2.vitesse
                    vitesse[0] = 0
                    bond.point2.vitesse = rotation.transpose() * vitesse
                    # bond.point2.position *= rotation
                    # bond.point2.position[0] = bond.l0
                    # bond.point2.position *= rotation.transpose()
            else:
                if isinstance(bond, Rod):
                    T = rotation * (F[bond.point1] + F[bond.point2])
                    T[1] = 0
                    T *= rotation.transpose()
                    F[bond.point1] += T
                    F[bond.point2] -= T
                    # F[bond.point1] *= rotation
                    # F[bond.point2] *= rotation
                    # Fn = F[bond.point1][0] + F[bond.point2][0]
                    # F[bond.point1][0] = Fn
                    # F[bond.point2][0] = Fn
                    # F[bond.point1] *= rotation.transpose()
                    # F[bond.point2] *= rotation.transpose()
                    vitesse1 = rotation * bond.point1.vitesse
                    vitesse2 = rotation * bond.point2.vitesse
                    v = (bond.point1.masse * vitesse1[0] + bond.point2.masse * vitesse2[0]) / (bond.point1.masse + bond.point2.masse)
                    vitesse1[0] = v
                    vitesse2[0] = v
                    bond.point1.vitesse = rotation.transpose() * vitesse1
                    bond.point2.vitesse = rotation.transpose() * vitesse2

        # Mise à jour des vitesses et des positions
        for movable in self.movables:
            movable.vitesse += dt * F[movable] / movable.masse
